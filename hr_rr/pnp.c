...
static const float32_t min_pnp_rt = 2.0;      //Optimum pnp value (result) must be = or > min_pnp
static const float32_t min_pnp_bg = 2.5;
...
void PnpBGTaskFn(void *argument)
{
	...
	//			pnp_data_table_t pnp_opti_xz;	
			...
			if((flag_lowsig_xz == true) || (pnp_opti_xz.np_ratio < 2.0) || (good_found_xz == false)){
